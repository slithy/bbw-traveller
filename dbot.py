...
class Avrae(commands.AutoShardedBot):
    ...
    @staticmethod
    def log_exception(exception=None, ctx: context.BambleweenyContext = None):
    ...
    async def get_context(self, *args, **kwargs) -> context.BambleweenyContext:
        return await super().get_context(*args, cls=context.BambleweenyContext, **kwargs)
...
bot = Avrae(
    ...
    activity=discord.Game(name=f"Traveller | {config.DEFAULT_PREFIX}help"),
