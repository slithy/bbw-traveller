...
class Library:
    ...
    def search(self, term):
        ...
        exact_match = [
            k for key, k in enumerate(LibraryData) if term.lower() == k.lower()
        ]

        # Otherwise, make a list of partial matches
        partial_match = [
            k for key, k in enumerate(LibraryData) if term.lower() in k.lower()
        ]
        ...
        if len(partial_match) == 1:
            return f"**Information.** Library data available on **{partial_match[0]}**:\n{LibraryData[partial_match[0]]}"
