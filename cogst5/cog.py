...
class Game(commands.Cog):
    ...
    async def query_library(self, ctx, search_term: str, *args):
        ...
        """
        search_term = f"{search_term} " + " ".join(args)
    ...
    async def load_ship(self, ctx, carouse_or_broker_or_streetwise_mod: int, brocker_or_streetwise_mod: int, SOC_mod: int, w_to_name: str):
        # load passengers
    ...
    async def ship_curr(self, ctx, compactness: int=1):
    ...
    async def set_ship_attr(self, ctx, attr_name: str, value):
        cs = self.session_data.get_ship_curr()
    ...
    async def travel_m(self, ctx, d_km: int=None, is_diam_for_jump: bool=False):
    ...
    async def consume_fuel(self, ctx, count: int):
        cs = self.session_data.get_ship_curr()
    ...
    async def set_world(self, ctx, name: str, uwp: str, zone: str, hex: str, sector: str=None):
        """Add a world"""
        if sector is None:
    ...
    async def set_world_attr(self, ctx, attr_name: str, value=None):
        cw = self.session_data.get_world()
    ...
    async def get_objs(self, ctx, name: str=""):
        cs = self.session_data.get_ship_curr()
    ...
    async def set_obj_attr(self, ctx, name: str, attr_name: str, value, cont: str=None):
        cs = self.session_data.get_ship_curr()
    ...
    async def add_debt(self, ctx, name: str, capacity: int, due_day: int, due_year: int, period: int=None, end_day: int=None, end_year: int=None):
        new_debt = BbwDebt(
            name=name,
            count=1,
    ...
    async def set_debt_attr(self, ctx, debt_name: int, attr_name: str, value):
    ...
    async def add_wish(self, ctx, name: str, capacity: int=None, TL: int=None, value: int=None):
        try:
    ...
    async def set_wish_attr(self, ctx, item_name: str, attr_name: str, value):
